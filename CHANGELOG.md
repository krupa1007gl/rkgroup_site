# Рефакторинг бэкенда и реализация концепции сайта — итоговый отчёт

Ветка `backend-refactor` от `main`.

## Реализация концепции сайта (фронт + бэкенд + API)

По финальной концепции (5 страниц + отдельный FAQ) сайт полностью
перестроен:

- **Удалено**: приложения `bots` (каталог ботов с ценами) и `news` —
  их нет в карте сайта концепции.
- **Главная** (`main:home`) — пересобрана по 9 блокам концепции: hero без
  хайпа, партнёрские бейджи (TWIN/Сколково) и логотипы клиентов (модель
  `Partner` теперь различает `partner_type`: `badge`/`client`), тизер
  живого диалога с CTA на AI Lab, карточки возможностей вместо выдуманных
  цифр результата, тизер «Продукты» (4 роли, без каталога), мостики-ссылки,
  мини-FAQ.
- **«О ботах»** (переиспользует `main:about`) — было «О компании» с
  командой, теперь простое объяснение AI-агента с тремя примерами диалогов.
- **AI Lab** (`ailab:page`, `/ailab/`) — фронт поверх уже готового API:
  автопроигрываемый сценарий (реалистичный черновик на тему ЖКХ) →
  приглашение → SMS-верификация (обязательный чекбокс согласия, honeypot,
  код-заглушка `0000`) → экран живого бота (`coming_soon` + форма
  «сообщить о запуске») → вкладки демо CRM/Excel.
- **«Партнёрам»** — переписана под white-label/со-бренд модель CRM-
  интеграторов вместо витрины логотипов.
- **«Кейсы»** — все 6 кейсов переписаны как обезличенные сценарии по
  отраслям, без вымышленных процентов результата и без привязки к
  неподтверждённым реальным компаниям (Тверская генерация/X5/Ростелеком/
  ПЭК/ТТК/2ГИС были в исходном контенте без каких-либо оснований считать
  их подтверждёнными клиентами).
- **FAQ** (`main:faq`, `/faq/`) — новая страница, 7 вопросов из концепции,
  доступна только через футер, не в верхнем меню.
- Хедер: телефон и почта видны на каждой странице напрямую (не в футере).

### Найденные и исправленные баги в процессе (не только по чек-листу)

- Маска телефона на AI Lab считала первую введённую цифру технической
  (унаследовано от старого паттерна) — теряла реальную цифру номера.
- `fetch()`-запросы на AI Lab падали с 403 — не отправляли CSRF-токен.
  Добавлен `{% csrf_token %}` + `X-CSRFToken` из cookie.
- Три шаблона (`partners.html`, `cases/case_detail.html` до правки, и
  позже мини-FAQ на `home.html`/`partners.html`) ссылались на CSS-классы
  из файлов, которые не подключали (`.cases-hero`, `.alert-*-custom`,
  `.faq-item`) — стили никогда не применялись. Общие классы вынесены в
  `base.css`, недостающие подключения `faq.css` добавлены.
- Парсинг `results` у модели `Case` ломался на разделителе `::` (парсер
  режет по первому одинарному `:`) — давал лишний символ перед значением.

### Что не входило в этот раунд

Контейнеризация (Dockerfile/docker-compose) — следующий шаг по явному
указанию владельца, после того как фронт одобрен.

### Проверка

`python manage.py test` — 46 тестов, все проходят (включая новые smoke-
тесты на все страницы карты сайта и на отсутствие вымышленных названий
компаний в кейсах). Полный цикл AI Lab проверен вручную через браузер
(не только через API) и подтверждён в БД.

---

# Рефакторинг бэкенда — итоговый отчёт (предыдущий раунд)

Ниже — что сделано по каждому пункту техзадания (A–D) в первом раунде
рефакторинга, что осталось как открытый вопрос для продукта, и как
проверить результат.
